  ...
  const advantages = [
    {
      ...
      title: t('landing.adv1Title'),
      description: t('landing.adv1Desc')
    },
    {
      icon: <Smartphone size={40} className="mb-4" />,
      title: t('landing.adv2Title'),
      description: t('landing.adv2Desc')
    },
    {
      icon: <BarChart3 size={40} className="mb-4" />,
      title: t('landing.adv3Title'),
      description: t('landing.adv3Desc')
    },
    {
      icon: <ShieldCheck size={40} className="mb-4" />,
      title: t('landing.adv4Title'),
      description: t('landing.adv4Desc')
    }
  ];

  return (
    <div className="landing-bg w-full min-h-screen bg-[#0a0e1a] text-[var(--text-primary)] font-sans relative overflow-x-hidden selection:bg-[var(--accent-primary)] selection:text-white">
      ...
      <div className="relative z-10">
        {/* Header */}
        <header className="landing-header sticky top-0 w-full flex justify-between items-center p-4 sm:p-6 md:px-12 z-50 bg-[#0a0e1a]/85 backdrop-blur-md border-b border-[rgba(59,130,246,0.2)] shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
          <div 
          ...
          >
            ...
            <span className="text-xl sm:text-2xl font-black tracking-tighter bg-clip-text text-transparent bg-gradient-to-r from-[var(--text-primary)] to-[var(--text-secondary)]">
              CashInc
            </span>
          </div>
          
          <nav className="hidden lg:flex gap-2 text-sm font-medium text-[var(--text-secondary)]">
            <a href="#que-es" className="nav-glow-link">{t('landing.navWhat')}</a>
            <a href="#pymes" className="nav-glow-link">{t('landing.navPymes')}</a>
            <a href="#ventajas" className="nav-glow-link">{t('landing.navAdvantages')}</a>
          </nav>
          ...
          <div className="flex gap-2 sm:gap-3 items-center">
            ...
            <Button onClick={() => navigate('/login', { state: { register: true } })} className="flex items-center gap-1 sm:gap-2 !rounded-full bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 text-white shadow-[0_0_20px_rgba(59,130,246,0.6)] border-none px-4 sm:px-6 md:px-8 py-2 sm:py-2.5 text-sm sm:text-base transition-all hover:scale-105">
              {t('landing.startBtn')} <ArrowRight size={16} className="sm:w-[18px] sm:h-[18px]" />
            </Button>
          </div>
        </header>
        ...
        <section className="flex flex-col items-center justify-center min-h-[80vh] text-center px-4 pt-8 sm:pt-10">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-7xl font-black tracking-tight mb-4 sm:mb-6 max-w-4xl leading-tight animate-slide-up" style={{ animationDelay: '0.1s' }}>
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-300">{t('landing.heroTitle2')}</span>
          </h1>
          
          <p className="text-base sm:text-lg md:text-xl text-[var(--text-secondary)] mb-8 sm:mb-10 max-w-2xl mx-auto animate-slide-up leading-relaxed px-2 sm:px-0" style={{ animationDelay: '0.2s' }}>
        ...
        <section id="que-es" className="px-4 sm:px-6 md:px-12 py-16 sm:py-20 max-w-6xl mx-auto">
          <h2 className="text-2xl sm:text-3xl font-bold text-center mb-8 sm:mb-12">{t('landing.whatTitle')}</h2>
          
          <div className="grid md:grid-cols-2 gap-6 sm:gap-8">
            <div className="landing-card bg-[#111827] border border-gray-800 rounded-[2rem] sm:rounded-3xl p-6 sm:p-10 neon-hover shadow-lg cursor-default">
              <h3 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-blue-400 border-b border-[var(--border-light)] pb-3 sm:pb-4">
                {t('landing.controlTitle')}
              </h3>
              <ul className="space-y-3 sm:space-y-4 text-sm sm:text-base text-[var(--text-secondary)]">
                <li className="flex items-start sm:items-center gap-3">
                  <CheckCircle2 className="text-green-500 shrink-0 mt-0.5 sm:mt-0" size={20} />
                  <span>{t('landing.controlItem1')}</span>
                </li>
                <li className="flex items-start sm:items-center gap-3">
                  <CheckCircle2 className="text-green-500 shrink-0 mt-0.5 sm:mt-0" size={20} />
                  <span>{t('landing.controlItem2')}</span>
                </li>
                <li className="flex items-start sm:items-center gap-3">
                  <CheckCircle2 className="text-green-500 shrink-0 mt-0.5 sm:mt-0" size={20} />
                  <span>{t('landing.controlItem3')}</span>
                </li>
              </ul>
            </div>
            
            <div className="landing-card bg-[#111827] border border-gray-800 rounded-[2rem] sm:rounded-3xl p-6 sm:p-10 neon-hover shadow-lg cursor-default">
              <h3 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-cyan-400 border-b border-[var(--border-light)] pb-3 sm:pb-4">
                {t('landing.idealTitle')}
              </h3>
              <ul className="space-y-3 sm:space-y-4 text-sm sm:text-base text-[var(--text-secondary)]">
                <li className="flex items-start sm:items-center gap-3">
                  <CheckCircle2 className="text-green-500 shrink-0 mt-0.5 sm:mt-0" size={20} />
                  <span>{t('landing.idealItem1')}</span>
                </li>
                <li className="flex items-start sm:items-center gap-3">
                  <CheckCircle2 className="text-green-500 shrink-0 mt-0.5 sm:mt-0" size={20} />
                  <span>{t('landing.idealItem2')}</span>
                </li>
                <li className="flex items-start sm:items-center gap-3">
                  <CheckCircle2 className="text-green-500 shrink-0 mt-0.5 sm:mt-0" size={20} />
                  <span>{t('landing.idealItem3')}</span>
                </li>
              </ul>
            </div>
          </div>
        ...
        <section id="pymes" className="w-full relative py-16 sm:py-24 my-6 sm:my-10 overflow-hidden">
          ...
          <div className="landing-section-overlay absolute inset-0 bg-gradient-to-r from-[#0a0e1a] via-[#0a0e1a]/90 to-[#0a0e1a]/70"></div>
          ...
          <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 md:px-12 flex flex-col md:flex-row items-center gap-10 sm:gap-16">
            <div className="w-full md:w-1/2 text-center md:text-left">
              <h2 className="text-3xl sm:text-4xl font-bold mb-4 sm:mb-6">{t('landing.pymesTitle')} <span className="text-blue-400">{t('landing.pymesHighlight')}</span></h2>
              <p className="text-[var(--text-secondary)] text-base sm:text-lg leading-relaxed mb-4 sm:mb-6">
                {t('landing.pymesText1')} <strong>{t('landing.pymesStrong1')}</strong> {t('landing.pymesText2')}
              </p>
              <p className="text-[var(--text-secondary)] text-base sm:text-lg leading-relaxed">
                {t('landing.pymesText3')}
              </p>
            </div>
            
            <div className="landing-purpose-card w-full md:w-1/2 bg-white/5 backdrop-blur-xl border border-white/10 p-6 sm:p-8 rounded-[2rem] sm:rounded-3xl shadow-2xl">
              <div className="flex items-center justify-center md:justify-start gap-3 sm:gap-4 mb-4 sm:mb-6">
                ...
                <h3 className="text-xl sm:text-2xl font-bold">{t('landing.purposeTitle')}</h3>
              </div>
              <p className="text-[var(--text-secondary)] text-base sm:text-lg leading-relaxed text-center md:text-left">
                {t('landing.purposeText1')} <strong>{t('landing.purposeStrong')}</strong>
              </p>
            </div>
          </div>
        </section>
        ...
        <section id="ventajas" className="px-6 md:px-12 py-20 max-w-7xl mx-auto">
          <h2 className="text-3xl font-bold text-center mb-16">{t('landing.advantagesTitle')}</h2>
            ...
            {advantages.map((adv, index) => (
              ...
              >
                ...
                <div className="w-full h-full absolute transition-all duration-500 preserve-3d group-hover:rotate-y-180 cursor-pointer">
                  ...
                  {/* Front of card */}
                  <div className="landing-flip-front absolute w-full h-full backface-hidden bg-[#111827]/80 backdrop-blur-md border border-[rgba(255,255,255,0.05)] rounded-2xl flex flex-col items-center justify-center p-6 text-center shadow-lg">
        ...
        {/* Footer */}
        <footer className="landing-footer border-t border-gray-800 py-12 mt-12 bg-black/50 backdrop-blur-md">
          <div className="max-w-7xl mx-auto px-6 text-center">
            ...
            <p className="text-2xl font-semibold text-[var(--text-secondary)] mb-2">
              {t('landing.footerSlogan')} <span className="text-blue-400">{t('landing.footerHighlight')}</span>
            </p>
            <p className="text-[var(--text-muted)] text-sm mt-8">{t('landing.footerCopy')}</p>
          </div>
        </footer>
      </div>
    </div>
